...
from app.schemas import MissionResponse
from app.models import Player, PlayerStatus, MissionAssigned
...
class PlayerRepository:
    ...
    async def get_player_by_user_and_lobby(
        self, user_id: UUID, lobby_id: UUID
    ) -> Player | None:
        """Get a player by user_id and lobby_id"""
        result = await self.db.execute(
            select(Player).where(
                and_(
                    Player.user_id == user_id,
