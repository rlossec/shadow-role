"""
Tests pour l'endpoint GET /api/games/{game_id}/missions.

Pour exécuter ces tests:
    uv run pytest tests/app/api/games/test_get_game_missions.py -v
"""
import pytest
...
from tests.app.api.games.helpers import get_game_missions_url


# 200 - Success
@pytest.mark.asyncio
async def test_get_game_missions_success(client, auth_service, db_session):
...
@pytest.mark.asyncio
async def test_get_game_missions_empty(client, auth_service, db_session):
...
@pytest.mark.asyncio
async def test_get_game_missions_invalid_uuid(client, auth_service):
    """Test récupération des missions avec un UUID invalide."""
    _, token = await create_user_and_get_token(client, auth_service)
    
    response = await client.get(
        get_game_missions_url("invalid-uuid"),
        headers=get_auth_headers(token)
    )
...
# 401 - Unauthorized
@pytest.mark.asyncio
async def test_get_game_missions_unauthorized(client, auth_service, db_session):
