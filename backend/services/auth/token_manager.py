...
class VerificationTokenManager:
    """Handle issuance and validation of email verification tokens."""
    ...
    async def create_token(self, user: User) -> str:
        token = secrets.token_urlsafe(48)
        expires_at = datetime.now(timezone.utc) + timedelta(hours=settings.VERIFY_TOKEN_EXPIRATION_HOURS)
        await self.user_repository.create_verification_token(user.id, token, expires_at)
        return token

    async def verify_token(self, token: str) -> Optional[VerificationToken]:
        verification_token = await self.user_repository.get_verification_token(token)
        if verification_token is None:
            return None
        if verification_token.used:
            return None
        if normalize_datetime(verification_token.expires_at) < datetime.now(timezone.utc):
            return None
        return verification_token

    async def mark_token_used(self, token: VerificationToken) -> None:
        await self.user_repository.mark_verification_token_used(token.id)
